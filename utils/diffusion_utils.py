...
def generate_image(pipe, prompt, num, seed=None, height=config.height, width=config.width,
                   num_inference_steps=config.num_inferenсe_steps, strength=config.strength,
    ...
    while num != 0:
        with autocast("cuda"):
            image = pipe(prompt=[prompt], negative_prompt=[neg], generator=generator, height=height, width=width,
